...
struct Edge {
  ...
  // contain source and destination and some weight. These 3 are elements in
  // this structure
...
struct Graph *createGraph(int V, int E) {
  ...
  // Creating "Edge" type structures inside "Graph" structure, the number of
  // edge type structures are equal to number of edges
...
void BellmanFord(struct Graph *graph, int source) {
  ...
  // This is initial step that we know , we initialize all distance to infinity
  // except source.
  ...
  // edge cycle. In this step we check for that shortest distances if graph
  // doesn't contain negative weight cycle.
...
int main() {
  ...
  struct Graph *graph =
      createGraph(V, E); // calling the function to allocate space to these many
                         // vertices and edges
